...
namespace BeardedManStudios.Forge.Networking.Unity
{
	public partial class NetworkManager : MonoBehaviour
	{
		...
		public GameObject[] WhaleburtNetworkNetworkObject = null;
		...
		private void CaptureObjects(NetworkObject obj)
		{
				...
				MainThreadManager.Run(() =>
				{
					...
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (NetworkManagerNetworkObject.Length > 0 && NetworkManagerNetworkObject[obj.CreateCode] != null)
						{
							...
							newObj = go.GetComponent<NetworkManagerBehavior>();
						}
					}
			...
			else if (obj is WhaleburtNetworkNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (WhaleburtNetworkNetworkObject.Length > 0 && WhaleburtNetworkNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(WhaleburtNetworkNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<WhaleburtNetworkBehavior>();
						}
					}
		...
		[Obsolete("Use InstantiateWhaleburtNetwork instead, its shorter and easier to type out ;)")]
		public WhaleburtNetworkBehavior InstantiateWhaleburtNetworkNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(WhaleburtNetworkNetworkObject[index]);
			var netBehavior = go.GetComponent<WhaleburtNetworkBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<WhaleburtNetworkBehavior>().networkObject = (WhaleburtNetworkNetworkObject)obj;
		...
		/// <summary>
		/// Instantiate an instance of NetworkManager
		/// </summary>
		/// <returns>
		/// A local instance of NetworkManagerBehavior
		/// </returns>
		/// <param name="index">The index of the NetworkManager prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		...
		/// <summary>
		/// Instantiate an instance of WhaleburtNetwork
		/// </summary>
		/// <returns>
		/// A local instance of WhaleburtNetworkBehavior
		/// </returns>
		/// <param name="index">The index of the WhaleburtNetwork prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public WhaleburtNetworkBehavior InstantiateWhaleburtNetwork(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(WhaleburtNetworkNetworkObject[index]);
			var netBehavior = go.GetComponent<WhaleburtNetworkBehavior>();
